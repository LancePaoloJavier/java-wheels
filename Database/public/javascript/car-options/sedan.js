// ===== sedan.js (UPDATED WITH BACKEND API INTEGRATION) =====
...
// Load sedans from backend
async function loadSedans() {
    try {
        const cars = await apiRequest('/cars?type=Sedan');
        renderSedans(cars);
    } catch (error) {
        console.error('Error loading sedans:', error);
        showToast('Failed to load sedan cars', 'error');
    }
}

// Render sedan cars dynamically
function renderSedans(cars) {
    ...
    if (cars.length === 0) {
        container.innerHTML = '<div class="col-12"><p class="text-center text-muted">No sedan cars available at the moment.</p></div>';
        return;
    }
    ...
    cardDiv.innerHTML = `
        <div class="card shadow">
            <div class="card-body text-center">
                ...
                <img src="${car.image_url}" alt="${car.name}" class="img-fluid my-3" style="height: 120px; object-fit: cover;">
...
// Load sedans when page loads
document.addEventListener('DOMContentLoaded', () => {
    // Wait a bit for the login script to load
    setTimeout(loadSedans, 100);
});