// ===== vans.js (UPDATED WITH BACKEND API INTEGRATION) =====
...
// Load Vans from backend
async function loadVans() {
    try {
        const cars = await apiRequest('/cars?type=Van');
        renderVans(cars);
    } catch (error) {
        console.error('Error loading Vans:', error);
        showToast('Failed to load van cars', 'error');
    }
}

// Render Van cars dynamically
function renderVans(cars) {
    ...
    if (cars.length === 0) {
        container.innerHTML = '<div class="col-12"><p class="text-center text-muted">No van cars available at the moment.</p></div>';
        return;
    }
    ...
    cardDiv.innerHTML = `
        <div class="card shadow">
            <div class="card-body text-center">
                ...
                <img src="${car.image_url}" alt="${car.name}" class="img-fluid my-3" style="height: 150px; object-fit: cover;">
